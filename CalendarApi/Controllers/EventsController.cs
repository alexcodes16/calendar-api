...
public class EventsController : ControllerBase{
    ...
    {
        // This is a check to ensure the event wasn't deleted by someone else
        // while you were trying to update it.
        if (!_context.Events.Any(e => e.Id == id)){return NotFound();}
        else{throw;}
    }

    return NoContent(); // This means success, but there's nothing to send back.
}
...
public async Task<IActionResult> AddParticipant(int eventId, [FromBody] int userId){
    // Find the event, and make sure to include its current list of participants
    ...
    // Add the user to the event's participants list and save
